...
def get_args():
    ...
    parser.add_argument("--rotations", type=bool, default=1,
...
def main():
    ...
    val_generator = ValGenerator(test_dir,gt_dir,nb_val_images = nb_val_images, rotations=rotations,sum_label=sum_label)
    ...
    for ii in range(3):
        ...
        axarr[0,0].imshow(np.squeeze(xg[ii,:,:,:]), vmin=0, vmax=np.max(xg[ii,:,:,:])*0.2)
        axarr[0,1].imshow(np.squeeze(yg[ii,:,:,:]), vmin=0, vmax=np.max(yg[ii,:,:,:])*0.9 if sum_label == "True" or gt_dir != "not_specified" else np.max(yg[ii,:,:,:])*0.2)
        axarr[1,0].imshow(np.squeeze(xv), vmin=0, vmax=np.max(xv)*0.2)
        axarr[1,1].imshow(np.squeeze(yv), vmin=0, vmax=np.max(yv)*0.9 if sum_label == "True" or gt_dir != "not_specified" else np.max(yv)*0.2)
